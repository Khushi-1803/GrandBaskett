...
const Cart = () => {
    ...
    const getCart = () => {
       let tempArray = []
       for(const key in cartItems){  //Loops through each key in cartItems. and Here, key is the product _id.
        const product = products.find((items)=>items._id === key) //Finds the product in your products array whose _id matches the current key.
        product.quantity = cartItems[key] //Adds a new property quantity to the product object. tells the qty of product
        tempArray.push(product)  //Pushes that updated product into a temporary array.
       }
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded overflow-hidden" onClick={()=>{navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)}}>
